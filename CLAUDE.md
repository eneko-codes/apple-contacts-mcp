...
Reading the address book is always fine. Do not modify or delete an existing contact or List membership. Test contacts and Lists in the owner's real address book are live data: they are allowed only under the exception below, and when allowed must be clearly named `TESTING: ...` and cleaned up when done.
