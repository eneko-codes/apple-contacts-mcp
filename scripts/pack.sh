...
#
# Both keys are checked by name rather than by grepping for "UsageDescription": the two
# permissions are independent, and a plist carrying only the Contacts string would pass
# a loose grep while leaving every note unreadable with no dialog to explain it.
check_usage_descriptions() {
  local binary="$1" stage="$2" key
  for key in NSContactsUsageDescription NSAppleEventsUsageDescription; do
    otool -P "$binary" | grep -q "$key" \
      || { echo "!! $key missing from the embedded Info.plist ($stage)" >&2; exit 1; }
  done
}

echo "==> Checking the embedded Info.plist survived linking"
check_usage_descriptions "$BINARY" "after linking"
...
# it blocks Apple events unless an entitlement opts back in. Resources/entitlements.plist
# carries that opt-in, which the note field depends on. Turn this on with MCPB_HARDENED=1
# when preparing something to distribute.
...
check_usage_descriptions "$STAGE/server/$NAME" "after signing"
